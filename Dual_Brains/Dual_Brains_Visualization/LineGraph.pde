        ...
        pushMatrix();
          ...
          for(int i = this.channels-1; i >= 0; i--){//for each channel...
            ...
            for(int j = 1; j < numOfReadingsStored; j++){//connect every point
              stroke(color(255,255,255,100*(numOfReadingsStored-j)/numOfReadingsStored));
              float baseWave = sin(TWO_PI * j/numOfReadingsStored) * cos(millis()*0.0001);
              curveVertex(scale * (numOfReadingsStored-j-1),(-1 * numOfReadingsStored * scale) / (channels+1) * (i+1) + (baseWave * (i+0.5) * 10) + (scale * sq(i)/2) + map(data[i][j-1], upperLim, lowerLim, 4 * scale, -4 * scale));
            
          //curveVertex(scale * (j-1),(-1 * numOfReadingsStored * scale) / (channels+1) * (i+1) + (baseWave * (i+0.5) * width/64) + (scale * sq(i)/2) + map(data[i][j-1], upperLim, lowerLim, 4 * scale, -4 * scale));
