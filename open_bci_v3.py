...
class OpenBCIBoard(object):
  ...
  def _read_serial_binary(self, max_bytes_to_skip=3000):
    def read(n):
      b = self.ser.read(n) #n = number of bytes to read
      if not b:
    ...
    for rep in range(max_bytes_to_skip):
      ...
      #---------Channel Data---------
        ...
        for c in range(self.eeg_channels_per_sample):
          ...
          #3 byte ints
          literal_read = read(3) #read the next 3 bytes
          # print(type(literal_read))
          unpacked = struct.unpack('3B', literal_read) # unpack the buffer, 3-byte unsigned character = (250,206,111)
          # print(unpacked)
          # log_bytes_in = log_bytes_in + '|' + str(literal_read); #just to log
          #3byte int in 2s compliment
          if (unpacked[0] >= 128):
            pre_fix = bytes(bytearray.fromhex('FF'))
          else:
            pre_fix = bytes(bytearray.fromhex('00'))
          # print('literal_read.decode()',literal_read.decode())
          literal_read = pre_fix +  literal_read
          # print(literal_read)
          # print('type lit read', type(literal_read))
  ...
  def warn(self, text):
    if self.log:
      ...
      # if len(self.log_packet_count) > 0:
